...
def check_all(predicate, iterable):
    """Alternative to builtin 'all'"""
    for i in iterable:
        if not predicate(i):
            return False
    return True
...
def set_handler(name: str, func: _Callable[..., None]) -> None:
    ...
    event_registry[name].add(lambda: func)#, _make_callback(name))
    ...
    #     event_registry[name].add((func, _make_callback(name)))
...
def has_components(entity: int, *component_types: _Type[_C]) -> bool:
    ...
    return check_all(lambda x: x in _entities[entity], component_types)
...
def _get_components(*component_types: _Type[_C]) -> _Iterable[_Tuple[int, _List[_C]]]:
    ...
    try:
        ...
        for entity in reduce(set.intersection, [comp_db[ct] for ct in component_types]):
...
def get_component(component_type: _Type[_C]) -> _List[_Tuple[int, _C]]:
    ...
    try:
        ...
        return dict_setdefault(_get_component_cache,
...
def get_components(*component_types: _Type[_Any]) -> _Iterable[_Tuple[int, _Tuple[_Any, ...]]]:
    ...
    try:
        ...
        return dict_setdefault(_get_components_cache,
...
def try_components(entity: int, *component_types: _Type[_C]) -> _Optional[_Tuple[_C, ...]]:
    ...
    """
    if check_all(lambda comp_type: comp_type in _entities[entity], component_types):
